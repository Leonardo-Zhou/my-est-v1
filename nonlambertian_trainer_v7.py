...
class NonLambertianTrainerV7:
    def __init__(self, options):
        ...
        # Convert scale lists to ranges for the decoder
        albedo_scales = list(self.opt.albedo_scales)
        shading_scales = list(self.opt.shading_scales)
        specular_scales = list(self.opt.specular_scales)
        
        # Sort in descending order to ensure proper processing
        albedo_scales.sort(reverse=True)
        shading_scales.sort(reverse=True)
        specular_scales.sort(reverse=True)
        ...
        self.models["decompose"] = networks.NonLambertianDecomposeDecoderV7(
            ...
            albedo_scales=albedo_range, shading_scales=shading_range, specular_scales=specular_range)
    ...
    def process_batch(self, inputs):
        ...
        outputs[("decompose_A"), 0], outputs[("decompose_S"), 0], outputs[("decompose_R"), 0] = \
            self.models["decompose"](decompose_features)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        # Reconstruction constraint: I = A*S + R
        reconstruction_loss = compute_reprojection_loss(
            inputs[("color", 0, 0)], outputs["reconstructed"])
        total_loss += self.opt.reconstruction_constraint * reconstruction_loss.mean()

        # Albedo consistency constraint
        albedo_loss = get_smooth_loss(outputs[("decompose_A"), 0], inputs[("color", 0, 0)])
        total_loss += self.opt.albedo_constraint * albedo_loss

        # Final reconstruction constraint
        final_reconstruction_loss = compute_reprojection_loss(
            inputs[("color", 0, 0)], outputs["reconstructed"])
        total_loss += self.opt.reprojection_constraint * final_reconstruction_loss.mean()

        # Specular smoothness/sparsity constraint
        specular_smoothness_loss = get_smooth_loss(outputs[("decompose_R"), 0], inputs[("color", 0, 0)])
        total_loss += self.opt.specular_smoothness * specular_smoothness_loss

        # L1 sparsity constraint for specular component
        specular_l1_loss = torch.abs(outputs[("decompose_R"), 0]).mean()
        total_loss += self.opt.specular_l1_sparsity * specular_l1_loss
