...
class NonLambertianTrainer:
    ...
    def compute_losses(self, inputs, outputs):
        ...
        loss_specular_l1 = torch.tensor(0.0, device=device)
        ...
        # Specular constraints (sparsity and smoothness)
        for frame_id in self.opt.frame_ids:
            specular = outputs[("specular", 0, frame_id)]
            # L2 smoothness for smooth regions
            loss_specular_smooth += torch.mean(specular * specular)
            # L1 sparsity constraint - encourage most values to be zero
        ...
        # Apply gradual specular weight based on training epoch
        specular_weight = getattr(self, 'current_specular_weight', 1.0)
        ...
        total_loss = (
            ...
            specular_weight * self.opt.specular_smoothness * loss_specular_smooth / num_all_frames +
            specular_weight * getattr(self.opt, 'specular_l1_sparsity', 0.05) * loss_specular_l1 / num_all_frames
        )
